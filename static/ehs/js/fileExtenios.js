...
const fileExtensions = {
  // 文本和文档文件
  textDocument: [
    '.txt', '.doc', '.docx', '.docm', '.dot', '.dotx', '.dotm',
    '.pdf', '.xps', '.oxps',
    '.rtf',
    '.odt', '.fodt', '.ott', '.odm',
    '.pages',
  ...
  // 虚拟化和磁盘映像
  virtualization: [
    '.iso', '.img', '.bin', '.nrg', '.mdf', '.ccd', '.sub',
    '.dmg', '.sparseimage', '.toast',
    '.vhd', '.vhdx', '.avhd', '.avhdx',
    '.vmdk', '.vmem', '.vmsd', '.vmsn', '.vmss', '.vmtm', '.vmx', '.vmxf',
    '.ova', '.ovf',
    '.qcow', '.qcow2', '.qed',
    '.hdd', '.hds'
  ],
  ...
  professional: [
    '.dwg', '.dxf', '.dwt', '.dws',
    '.stl', '.obj', '.fbx', '.dae', '.3ds', '.blend', '.max', '.ma', '.mb',
    '.iges', '.igs', '.step', '.stp',
    '.shp', '.shx', '.dbf', '.prj', '.sbn', '.sbx', '.shp.xml',
    '.kml', '.kmz',
    '.gpx',
    '.tiff', '.tif', '.geotiff', '.geotif',
    '.mbtiles',
    '.fits', '.fit',
    '.hdf', '.h4', '.hdf4', '.h5', '.hdf5', '.he5',
    '.nc', '.cdf',
    '.mat',
    '.sav', '.zsav',
    '.root',
...
const fileExtensionUtils = {
  ...
  isImageFile(filename) {
    return fileExtensions.image.some(ext => 
      filename.toLowerCase().endsWith(ext)
    );
  },

  isAudioFile(filename) {
    return fileExtensions.audio.some(ext => 
      filename.toLowerCase().endsWith(ext)
    );
  },

  isVideoFile(filename) {
    return fileExtensions.video.some(ext => 
      filename.toLowerCase().endsWith(ext)
    );
  },

  isArchiveFile(filename) {
    return fileExtensions.archive.some(ext => 
      filename.toLowerCase().endsWith(ext)
    );
  },

  isExecutableFile(filename) {
    return fileExtensions.executable.some(ext => 
      filename.toLowerCase().endsWith(ext)
    );
  },

  isSourceCodeFile(filename) {
    return fileExtensions.sourceCode.some(ext => 
      filename.toLowerCase().endsWith(ext)
    );
  },
