...
async function simulateUpload(image, moduleId, times) {
  ...
  const response = await axios.post("/api/SaveAfterPhotos/", formData, { timeout: TIMEOUT });
...
async function uploadSelectedImages(modules, moduleId, times, retryCount = 0) {
  ...
  for (let i = 0; i < selectedImages.length; i++) {
    ...
    console.log(image);
    objData.push({
      [image.formData.obj.CheckAreaCode]: { CheckAreaCode: image.formData.obj.CheckAreaCode, ct_code: image.formData.obj.TemplateList[0].CTCode, path: image.name },
    });
...
async function saveObject(objData) {
